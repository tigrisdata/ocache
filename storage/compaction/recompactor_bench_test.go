...
const (
	...
	benchmarkSegmentLiveWitnessPrefix = "!segment-live-witness/"
	benchmarkSegmentCoveredPrefix     = "!segment-live-covered/"
	benchmarkSegmentLiveVersion       = byte(1)
	benchmarkSegmentCoveredVersion    = byte(2)
)

func benchmarkSegmentLiveKey(path string, offset int64) []byte {
	return benchmarkSegmentLiveLocationKey(benchmarkSegmentLivePrefix, path, offset)
}

func benchmarkSegmentLiveWitnessKey(path string, offset int64) []byte {
	return benchmarkSegmentLiveLocationKey(benchmarkSegmentLiveWitnessPrefix, path, offset)
}

func benchmarkSegmentLiveLocationKey(namespace, path string, offset int64) []byte {
	encoded := base64.RawURLEncoding.EncodeToString([]byte(path))
	key := make([]byte, len(namespace)+len(encoded)+1+8)
	copy(key, namespace)
	copy(key[len(namespace):], encoded)
	key[len(namespace)+len(encoded)] = '/'
...
func benchmarkSegmentCoveredValue(entries uint32, dataBytes, size int64) []byte {
	...
	value[0] = benchmarkSegmentCoveredVersion
...
func BenchmarkRecompactFragmentedSegment(b *testing.B) {
	for _, dead := range []int{8, 64, 512, 4096} {
...
func createBenchmarkRecompactionSegment(tb testing.TB, sm *segment.Manager, meta *metadata.MetaDB, dead int) (*segment.Segment, error) {
	...
	for key, value := range entries {
		...
		if len(key) >= len("live-") && key[:len("live-")] == "live-" {
			...
			row := benchmarkSegmentLiveRow(
				key, int64(len(value)), segment.CalculateValueHeaderSize(key), 0, segment.CurrentValueHeaderVersion,
			)
			batch.Put(benchmarkSegmentLiveKey(seg.Path(), offset), row)
			batch.Put(benchmarkSegmentLiveWitnessKey(seg.Path(), offset), row)
		}
	}
