...
func TestSegmentLiveIndexKeyRoundTripAndOffsetOrder(t *testing.T) {
	...
	for i, want := range []int64{0, 128, 4096} {
		...
		witnessPath, witnessOffset, witnessOK := ParseSegmentLiveIndexWitnessKey(MakeSegmentLiveIndexWitnessKey(path, want))
		require.True(t, witnessOK)
		require.Equal(t, path, witnessPath)
		require.Equal(t, want, witnessOffset)
	}
}
...
func TestSegmentLiveCoverageEncoding(t *testing.T) {
	want := SegmentLiveIndexCoverage{Entries: 17, DataBytes: 987654, Size: 1234567, HasWitness: true}
	encoded, err := EncodeSegmentLiveIndexCoverage(want)
	require.NoError(t, err)
	require.Equal(t, byte(SegmentLiveCoverageVersion), encoded[0])
	got, err := DecodeSegmentLiveIndexCoverage(encoded)
	require.NoError(t, err)
	require.Equal(t, want, got)

	legacy := append([]byte(nil), encoded...)
	legacy[0] = SegmentLiveIndexVersion
	got, err = DecodeSegmentLiveIndexCoverage(legacy)
	require.NoError(t, err)
