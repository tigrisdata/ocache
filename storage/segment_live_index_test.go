...
func createIndexedSegmentValue(t *testing.T, s *Storage, key string, payload []byte, expiry int64) (*segment.Segment, *pb.ValueMessage) {
	...
	batch.Put(keys.MakeSegmentLiveIndexWitnessKey(seg.Path(), offset), indexValue)
...
func segmentLiveWitnessRowExists(t *testing.T, s *Storage, segmentPath string, offset int64) bool {
	t.Helper()
	ro := grocksdb.NewDefaultReadOptions()
	defer ro.Destroy()
	slice, err := s.meta.Handle().Get(ro, keys.MakeSegmentLiveIndexWitnessKey(segmentPath, offset))
	require.NoError(t, err)
	exists := slice.Exists()
	slice.Free()
	return exists
