...
- **Publication races**: the danger is walking a segment whose entries' metadata rows still sit in an in-flight compaction run's uncommitted batch — they would derive as dead, and reclaiming the segment would leave the pending commit publishing rows that point at a deleted file. The age gate is *not* a sufficient defence here: a run iterates the whole compaction index with no per-run byte cap, so under a large backlog and a rate-limited copy it can stay open for hours — longer than the 2 h gate. The compactor therefore **commits its metadata batch at every segment rollover, before finalizing the segment**, making "a closed segment's metadata is published" a structural invariant rather than a timing assumption. Since the walk only considers closed segments, the hazard is eliminated by construction (and crash recovery improves too: less work is lost and fewer segment bytes are orphaned per crash).
