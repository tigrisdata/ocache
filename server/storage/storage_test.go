...
func TestStorage_PutGetDelete_SmallObject(t *testing.T) {
	dir := t.TempDir()
	s, err := newStorage(dir, 3600, 1024, 4096, 16*1024*1024, 1000, 1024*1024)
...
func TestStorage_PutGetDelete_LargeObject(t *testing.T) {
	dir := t.TempDir()
	s, err := newStorage(dir, 3600, 8, 4096, 16*1024*1024, 1000, 1024*1024) // very low threshold to force spill
	assert.NoError(t, err, "failed to create storage")
	key := "largekey"
