...
// CacheService.GetTopology benchmark. The descriptors use the same dskit reader
// path as production; the long heartbeat timeout keeps this static fixture
// healthy for the duration of a profile run.
func NewTopologyBenchmarkManager(activeNodes, tokensPerNode int) (*RingManager, func(), error) {
	if activeNodes <= 0 {
		return nil, nil, fmt.Errorf("active nodes must be positive: %d", activeNodes)
	}
	if tokensPerNode <= 0 {
		return nil, nil, fmt.Errorf("tokens per node must be positive: %d", tokensPerNode)
	}
	if uint64(activeNodes)*uint64(tokensPerNode) > uint64(^uint32(0))+1 {
		return nil, nil, fmt.Errorf("%d nodes with %d tokens exceed the uint32 token space", activeNodes, tokensPerNode)
	}
	...
	for node := range activeNodes {
		...
		for token := range tokens {
			tokens[token] = uint32(node*tokensPerNode + token)
		}
		desc.AddIngester(
			fmt.Sprintf("node-%d", node),
			fmt.Sprintf("127.0.0.1:%d", 10000+node),
			"",
			tokens,
			dskitring.ACTIVE,
			registeredAt,
