...
# Compile the segment recompaction benchmark once so paired runs do not include
# Go compilation. PERFLOOP_BUILD_OUTPUT_DIR is supplied by the benchmark runner.
.PHONY: build-bench-recompaction
build-bench-recompaction:
	@test -n "$(PERFLOOP_BUILD_OUTPUT_DIR)" || { echo "PERFLOOP_BUILD_OUTPUT_DIR is required"; exit 1; }
	@mkdir -p "$(PERFLOOP_BUILD_OUTPUT_DIR)"
	@cd storage/compaction && CGO_ENABLED=1 CGO_CFLAGS="$(CGO_CFLAGS)" CGO_LDFLAGS="$(CGO_LDFLAGS)" go test $(LDFLAGS) -c -o "$(PERFLOOP_BUILD_OUTPUT_DIR)/recompaction.test" .
...
help:
	...
	@echo "  build-bench-recompaction - Compile the segment recompaction benchmark (set PERFLOOP_BUILD_OUTPUT_DIR)"
