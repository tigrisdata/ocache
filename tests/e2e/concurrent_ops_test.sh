...
echo "=== OCache Concurrent Operations E2E Test ==="
echo
...
echo "Verifying all 100 keys were written..."
KEY_COUNT=$(./ocachecli list | wc -l)
if [ "$KEY_COUNT" -eq 100 ]; then
    echo -e "${GREEN}✓ All 100 keys written successfully${NC}"
...
for reader in {1..10}; do
    (
        ...
        echo "$ERROR_COUNT" > "/tmp/ocache-concurrent-test-errors-${reader}"
    ) &
    READER_PIDS+=($!)
done
...
# Collect errors
for reader in {1..10}; do
    if [ -f "/tmp/ocache-concurrent-test-errors-${reader}" ]; then
        if ERRORS=$(cat "/tmp/ocache-concurrent-test-errors-${reader}" 2>/dev/null); then
            TOTAL_ERRORS=$((TOTAL_ERRORS + ERRORS))
        fi
        rm -f "/tmp/ocache-concurrent-test-errors-${reader}"
    fi
done

if [ "$TOTAL_ERRORS" -eq 0 ]; then
    echo -e "${GREEN}✓ All reads returned correct values${NC}"
...
MIXED_COUNT=$(./ocachecli list | grep -c "^mixed" || true)
...
if [ "$VERIFY_ERRORS" -eq 0 ]; then
    echo -e "${GREEN}✓ All updated values are correct${NC}"
...
for client in {1..5}; do
    (
        ...
        echo "$ERROR_COUNT" > "/tmp/ocache-raw-errors-${client}"
    ) &
    RAW_PIDS+=($!)
done
...
# Collect errors
for client in {1..5}; do
    if [ -f "/tmp/ocache-raw-errors-${client}" ]; then
        if ERRORS=$(cat "/tmp/ocache-raw-errors-${client}" 2>/dev/null); then
            RAW_ERRORS=$((RAW_ERRORS + ERRORS))
        fi
        rm -f "/tmp/ocache-raw-errors-${client}"
    fi
done

if [ "$RAW_ERRORS" -eq 0 ]; then
    echo -e "${GREEN}✓ Read-after-write consistency maintained${NC}"
...
REMAINING=$(./ocachecli list | grep -c "delete-test" || true)
if [ "$REMAINING" -eq 0 ]; then
    echo -e "${GREEN}✓ All keys successfully deleted${NC}"
