...
# wait_for_protection_outcome waits for the cleaner to reach the policy-specific
# survivor set without reading keys (reads would refresh LRU recency while the
# assertion is still settling). Successful puts establish that all expected keys
# were present; do not require one list to observe all of them because the cleaner
# may run while the second batch is still being written. The last observed counts
# are left in the global variables used below.
wait_for_protection_outcome() {
    local policy="$1"
    local deadline=$((SECONDS + 30))

    while [ "$SECONDS" -lt "$deadline" ]; do
        local listed
        listed=$(./ocachecli list 2>/dev/null || true)
        ORDER_EXISTS=$(printf '%s\n' "$listed" | grep -Ec '^order-key-([1-9]|1[0-9]|2[0-9]|30)$' || true)
        NEWEST_EXISTS=$(printf '%s\n' "$listed" | grep -Ec '^order-key-(26|27|28|29|30)$' || true)
        READ_OLD_EXISTS=$(printf '%s\n' "$listed" | grep -Ec '^order-key-([1-5])$' || true)

        # Every order-key put below must have succeeded. A missing key therefore
        # proves that the cleaner has run, even if no poll caught the transient
        # all-30 state.
        if [ "$ORDER_EXISTS" -lt 30 ] && [ "$NEWEST_EXISTS" -ge 3 ]; then
            if [ "$policy" = "lru" ] && [ "$READ_OLD_EXISTS" -ge 3 ]; then
                return 0
            fi
            if [ "$policy" = "fifo" ] && [ "$READ_OLD_EXISTS" -le 2 ]; then
                return 0
            fi
        fi
        sleep 1
...
run_eviction_suite() {
    ...
    if [ "$setup_ok" != true ]; then
        ...
        if [ "$setup_ok" != true ]; then
            fail_test "RESULT_${policy}_protection" "[${policy}] setup could not reliably write all order keys; protection check inconclusive"
        elif ! wait_for_protection_outcome "$policy"; then
            fail_test "RESULT_${policy}_protection" "[${policy}] cleaner did not produce a settled survivor set (order keys observed=$ORDER_EXISTS/30)"
        else
